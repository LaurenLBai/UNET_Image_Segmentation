...
import gc
from tqdm import tqdm
from predict_utils import get_loaders
...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    # result = np.empty([a*b, c, d, 1])
    # loader = get_loaders(subdivs, batch_size = 16, num_workers=4)
    # loop = tqdm(loader)
    # print('hello')
    # for batch_idx, batch in enumerate(loop):
    #     batch = batch.to(DEVICE)
    #     pred = pred_func(batch)
    #     pred = to_numpy(pred) 
    #     result[batch_idx] = input[:16][:][:][:]


    result = np.empty([a*b, c, d, 1])
    batch_id = 16
    for i in tqdm(range(subdivs.shape[0])):
        # section = subdivs[:batch_id][:][:][:]
        # print("initial subdiv shape: ", section.shape)
        input = to_tensor(subdivs[i])
        print("subdiv shape after to_tensor: ", input.shape)
        input = torch.from_numpy(input).to(DEVICE).unsqueeze(0)
        print("subdiv shape after tensor: ", input.shape)
        input = pred_func(input)
        print("shape after pred: ", input.shape)
        input = to_numpy(input) 
        mask_to_image(input)
        # print("shape after tonumpy: ", input.shape)
        input = np.array([patch * WINDOW_SPLINE_2D for patch in input])
        # print("shape after numpy array: ", input.shape)
        gc.collect()
        result[i] = input[0][:][:][:]
        batch_id+=16
    pred = np.array([patch * WINDOW_SPLINE_2D for patch in pred])
    # Such 5D array:
    result = result.reshape(a, b, c, d, nb_classes)
